...
func (w *Writer) writeCSV(out io.Writer) error {
	...
	if w.Cfg.Verbose {
		if err := writer.Write([]string{"method", "status", "target", "preview", "curl"}); err != nil {
			return err
		}
		for _, result := range w.VerboseResults {
			if err := writer.Write([]string{safeCSVField(result.Method), fmt.Sprint(result.Status), safeCSVField(result.Target), safeCSVField(result.Preview), safeCSVField(result.Curl)}); err != nil {
				return err
			}
		}
	} else {
		if err := writer.Write([]string{"method", "status", "target"}); err != nil {
			return err
		}
		for _, result := range w.Results {
			if err := writer.Write([]string{safeCSVField(result.Method), fmt.Sprint(result.Status), safeCSVField(result.Target)}); err != nil {
				return err
			}
		}
	}
