...
func writeTerminal(report Report, out io.Writer, colorMode string) error {
	...
	if _, err := fmt.Fprintf(out, "Brute coverage: %d URLs tested, %d wildcard false positives filtered, %d request errors, %d transport-limited targets, %d WAF-challenged targets (%d responses), %d references rejected by policy, %d skipped by limits; automate coverage gaps: %d\n", report.Metrics.BruteURLsTested, report.Metrics.BruteFalsePositivesFiltered, report.Metrics.BruteRequestErrors, report.Metrics.TransportLimitedTargets, report.Metrics.WAFChallengedTargets, report.Metrics.WAFChallengeResponses, report.Metrics.BruteReferencesRejected, report.Metrics.BruteReferencesSkipped, report.Metrics.Failures); err != nil {
		return err
	}
...
<section class="cards"><div class="card"><span class="muted">Targets</span><strong>{{.Metrics.Targets}}</strong></div><div class="card"><span class="muted">Specifications</span><strong>{{.Metrics.DiscoveredSpecifications}}</strong></div><div class="card"><span class="muted">Brute URLs tested</span><strong>{{.Metrics.BruteURLsTested}}</strong></div><div class="card"><span class="muted">False positives filtered</span><strong>{{.Metrics.BruteFalsePositivesFiltered}}</strong></div><div class="card"><span class="muted">WAF-challenged targets</span><strong>{{.Metrics.WAFChallengedTargets}}</strong></div><div class="card"><span class="muted">WAF challenge responses</span><strong>{{.Metrics.WAFChallengeResponses}}</strong></div><div class="card"><span class="muted">References rejected</span><strong>{{.Metrics.BruteReferencesRejected}}</strong></div><div class="card"><span class="muted">References skipped</span><strong>{{.Metrics.BruteReferencesSkipped}}</strong></div><div class="card"><span class="muted">API operations</span><strong>{{.Metrics.Operations}}</strong></div><div class="card"><span class="muted">Active fuzz probes</span><strong>{{.Metrics.ActiveProbes}}</strong></div><div class="card"><span class="muted">Coverage gaps</span><strong>{{.Metrics.Failures}}</strong></div><div class="card"><span class="muted">2xx rate</span><strong>{{printf "%.2f%%" .Metrics.SuccessRate}}</strong></div><div class="card"><span class="muted">5xx rate</span><strong>{{printf "%.2f%%" .Metrics.ServerErrorRate}}</strong></div><div class="card"><span class="muted">Weighted points</span><strong>{{.Severity.WeightedPoints}}</strong></div></section>
...
<h2>Prioritized findings</h2>{{range .Findings}}<article class="finding"><h3><span class="sev {{.Severity}}">{{upper (printf "%s" .Severity)}}</span> — {{.ID}}: {{.Title}}</h3><div class="meta"><span>Affected: {{.Count}}</span><span>Points: {{.WeightedPoints}}</span><span>Confidence: {{.Confidence}}</span><span>OWASP: {{join .OWASP}}</span></div><p>{{.Description}}</p><p><strong>Recommendation:</strong> {{.Recommendation}}</p>{{if .Evidence}}<table><thead><tr><th>Source</th><th>Method</th><th>Status</th><th>Target / note</th></tr></thead><tbody>{{range .Evidence}}<tr><td>{{.Source}}</td><td>{{.Method}}</td><td>{{if .Status}}{{.Status}}{{end}}</td><td>{{.Target}} {{.Note}}</td></tr>{{end}}</tbody></table>{{range .Evidence}}{{if or .ResponseBody .RequestBody .ResponseTruncated}}<details class="proof"><summary>Response proof — {{.Method}} {{if .Status}}{{.Status}}{{end}} {{if .URL}}{{.URL}}{{else}}{{.Target}}{{end}}</summary><div class="proof-body"><div class="proof-meta">{{if .Identity}}<span>Identity: {{.Identity}}</span>{{end}}{{if .Case}}<span>Case: {{.Case}}</span>{{end}}{{if .ContentType}}<span>Content-Type: {{.ContentType}}</span>{{end}}{{if .ResponseTruncated}}<span class="medium">Response truncated</span>{{else}}<span class="muted">Response complete</span>{{end}}</div>{{if .Guidance}}<div class="proof-label">Response-guided analysis</div><pre>{{.Guidance}}</pre>{{end}}{{if .RequestBody}}<div class="proof-label">Request body</div><pre>{{.RequestBody}}</pre>{{end}}<div class="proof-label">Response body</div><pre>{{.ResponseBody}}</pre></div></details>{{end}}{{end}}{{end}}</article>{{end}}
